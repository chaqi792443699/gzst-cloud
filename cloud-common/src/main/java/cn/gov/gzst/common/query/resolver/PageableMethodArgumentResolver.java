...
package cn.gov.gzst.common.query.resolver;

import com.google.common.collect.Lists;

import cn.gov.gzst.common.common.form.BaseMethodArgumentResolver;
import cn.gov.gzst.common.query.annotation.PageableDefaults;
import cn.gov.gzst.common.query.data.PageRequest;
import cn.gov.gzst.common.query.data.Pageable;
import cn.gov.gzst.common.query.data.Sort;
import cn.gov.gzst.common.query.data.Direction;
...
public class PageableMethodArgumentResolver extends BaseMethodArgumentResolver {
		...
		try {
			...
			for (String name : sortMap.keySet()) {
				...
				if (!StringUtils.isEmpty(value)) {
					...
					Direction direction = Direction.fromString(sortMap.get(name)[0]);
			...
			for (OrderedSort orderedSort : orderedSortList) {
				Sort newSort = new Sort(Direction.DESC, orderedSort.property);
				if (sort == null) {
					sort = newSort;
				} else {
	...
	private Pageable defaultPageable(PageableDefaults pageableDefaults) {
		...
		for (String sortStr : sortStrArray) {
			...
			Sort newSort = new Sort(Direction.fromString(sortStrPair[1]), sortStrPair[0]);
			if (sort == null) {
				sort = newSort;
			} else {
				sort = sort.and(newSort);
	...
	static class OrderedSort implements Comparable<OrderedSort> {
		...
		OrderedSort(String property, Direction direction, int order) {

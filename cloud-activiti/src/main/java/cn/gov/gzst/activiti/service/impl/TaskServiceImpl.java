...
import cn.gov.gzst.activiti.custom.cmd.CreateAndTakeTransitionCmd;
import cn.gov.gzst.activiti.custom.cmd.JumpTaskCmd;
import cn.gov.gzst.activiti.custom.creator.ChainedActivitiesCreator;
import cn.gov.gzst.activiti.custom.creator.MultiInstanceActivityCreator;
import cn.gov.gzst.activiti.custom.creator.RuntimeActivityDefinitionEntityIntepreter;
import cn.gov.gzst.activiti.custom.creator.SimpleRuntimeActivityDefinitionEntity;
...
import cn.gov.gzst.activiti.utils.ProcessDefUtils;
import cn.gov.gzst.activiti.utils.UserCacheUtils;
//import cn.gov.gzst.common.utils.UserUtils;
...
public class TaskServiceImpl implements ITaskService {
	...
	public List<Act> todoList(Act act){
		String userId = act.getAssignee();
		if (StringUtils.isEmpty(userId)){
			userId = "";//UserUtils.getUid();
		}
		...
		TaskQuery todoTaskQuery = taskService.createTaskQuery()
				...
				.desc();
		if (!StringUtils.isEmpty(userId)){
			todoTaskQuery.taskAssignee(userId);
		}
		// 设置查询条件
		...
		if (StringUtils.isNotBlank(act.getTaskDefKey())){
			toClaimQuery.taskDefinitionKey(act.getTaskDefKey());
		}
		if (act.getBeginDate() != null){
	...
	public List<Act> historicList(Act act){
		...
		HistoricTaskInstanceQuery histTaskQuery = historyService.createHistoricTaskInstanceQuery()
				...
				.desc();
		if (!StringUtils.isEmpty(userId)){
			histTaskQuery.taskAssignee(userId).finished();
		}
		// 设置查询条件
		if (StringUtils.isNotBlank(act.getProcDefKey())){
		...
		if (StringUtils.isNotBlank(act.getTaskDefKey())){
			histTaskQuery.taskDefinitionKey(act.getTaskDefKey());
		}
		if (act.getBeginDate() != null){
	...
	public Page<Act> historicList(Page<Act> page, Act act){
		...
		// 查询总数
		page.setTotal(Integer.parseInt(histTaskQuery.count()+""));
		int firstResult = (page.getCurrent() - 1) * page.getSize();
		if (firstResult >= page.getTotal()) {
			firstResult = 0;
		}
		// 查询列表
		List<HistoricTaskInstance> histList = histTaskQuery.listPage(firstResult, page.getSize());
		//处理分页问题
		List<Act> actList=Lists.newArrayList();
		for (HistoricTaskInstance histTask : histList) {
			Act e = new Act();
			...
			if (StringUtils.isNotBlank(histIns.getAssignee())
				...
				if ("startEvent".equals(histIns.getActivityType())){
					...
					if (il.size() > 0){
						if (StringUtils.isNotBlank(il.get(0).getStartUserId())){
							...
							UserCacheUtils.addUserId(histIns.getAssignee());
						}
					}
				}
				// 获取任务执行人名称
				...
				if (StringUtils.isNotBlank(histIns.getTaskId())){
					...
					if (commentList.size()>0){
						...
						if (!cn.gov.gzst.common.utils.StringUtils.isEmpty(comment)){
							String[] comments = comment.split("::");
							if (comments.length>1){
								e.setTitle(comments[0]);
								e.setComment(comments[1]);
							}else{
	...
	public Page<Object[]> processList(Page<Object[]> page, String category) {
		...
		page.setTotal(Integer.parseInt(processDefinitionQuery.count()+""));
		int firstResult = (page.getCurrent() - 1) * page.getSize();
		if (firstResult >= page.getTotal()) {
			firstResult = 0;
		}
	    List<ProcessDefinition> processDefinitionList = processDefinitionQuery.listPage(firstResult, page.getSize());
	    for (ProcessDefinition processDefinition : processDefinitionList) {
	      String deploymentId = processDefinition.getDeploymentId();
	      Deployment deployment = repositoryService.createDeploymentQuery().deploymentId(deploymentId).singleResult();
	      page.getRecords().add(new Object[]{processDefinition, deployment});
	    }
		return page;
	}
	...
	public void complete(String taskId, String procInsId, String comment, String title, Map<String, Object> vars){
		// 添加意见
		if (StringUtils.isNotBlank(comment)||StringUtils.isNotBlank(title)){
			...
			if (StringUtils.isNotBlank(procInsId)){
				procInsId = getTask(taskId).getProcessInstanceId();
			}
		...
		String eid = "";//UserUtils.getEid();
		// 设置流程标题
		if (StringUtils.isNotBlank(eid)){
			vars.put("eid", eid);
		}
		// 设置流程提交用户
		if (!vars.containsKey("submitEid")
				&&StringUtils.isNotBlank(userId)){
			vars.put("submitEid", eid);
		}
		// 设置流程提交用户
		if (!vars.containsKey("submitUid")
				&&StringUtils.isNotBlank(userId)){
			vars.put("submitUid", userId);
		}
		// 提交任务
...
//	}
	...
	public InputStream tracePhoto(String processDefinitionId, String executionId) {
		...
		Context.setProcessEngineConfiguration(springProcessEngineConfiguration);

package cn.gov.gzst.sso.server.controller.oauth;

import cn.gov.gzst.admin.bean.User;
import cn.gov.gzst.admin.feign.AppFeign;
import cn.gov.gzst.admin.feign.UserFeign;
import cn.gov.gzst.sso.server.Constants;
import cn.gov.gzst.sso.server.entity.Status;
import cn.gov.gzst.sso.server.service.OAuthService;
...
public class AuthorizeController {
	...
	public Object authorize(Model model, HttpServletRequest request ) throws URISyntaxException, OAuthSystemException {
		...
		try {
			...
			if ( !login(request) ) {
				...
				model.addAttribute("client", appFeign.findByAppKey(oauthRequest.getClientId()));
				return "oauth2login";
			}
	...
	private boolean login( HttpServletRequest request ) {
		...
		try {
			...
			if ( user != null ) {
				if ( !userFeign.checkUser(username, password) ) {
